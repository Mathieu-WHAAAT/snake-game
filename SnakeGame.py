...
def display_highscore(score, bg, font_menu):
    """ display high score """
    mouse = pygame.mouse.get_pos()
    surf_menu = font_menu.render(' HIGH SCORES ', True, VERTP, BLACK)
    rect_menu = surf_menu.get_rect()
    rect_menu.centerx = s_screen // 2
    rect_menu.top = 0.025*s_screen
    text_score = 'Current high score: '
    font_score = pygame.font.Font('freesansbold.ttf', 30)
    surf_score = font_score.render(text_score + str(score), True, VERTP, BLACK)
    rect_score = surf_score.get_rect()
    rect_score.center = (s_screen/2, s_screen*1/3)
    ...
    rect_back.top = 0.03*s_screen
    rect_back.left = 0.025*s_screen
...
def display_start_screen(bg, size_s, text_screen, text_screen2, font_screen, color):
    ...
    text_marque = " by Mathieu Dario "
    font_marque = pygame.font.Font('freesansbold.ttf', 20)
    surf_marque = font_marque.render(text_marque, True, GREEN)
    rect_marque = surf_marque.get_rect()
    rect_marque.center = (s_screen/2, 19/20*s_screen)
    DISPLAYSURF.blit(surf_screen, rect_screen)
    DISPLAYSURF.blit(surf_screen2, rect_screen2)
    DISPLAYSURF.blit(surf_marque, rect_marque)
...
while True:
    if jeu == 1: #start screen
        ...
        if setting == 1: # PLAY
            ...
            time_sec += 1/FPS
            # DISPLAY
            back.append(display_highscore(high_score, img_menu, font_menu))
        ...
        if time_sec >= boost: #updating snake
            xpos, ypos = Serpents_list[-1].get_pos()
            new_serp = Snake(xpos, ypos, 'body')
            Serpents_list = update_serpents(Serpents_list, displacement)
            lock = False
##            posx = xpos - Serpents_list[-1].get_pos()[0]
##            posy = ypos - Serpents_list[-1].get_pos()[1]
##            new_serp2 = Snake(posx + xpos, posy + ypos, 'body')
...
##                Serpents_list.append(new_serp2)
        ...
        if setting == 1: # PLAY
            ...
            Plateau, i_Serpent, displacement, Pomme, vitesse = initialise_all()
            score = 0
            Serpents_list = [i_Serpent]
            jeu = 1
            time_sec = 0.0
            continue
        else:
